...
# ── dependencies ──────────────────────────────────────────────
# Refresh package databases first (fresh systems have empty DBs)
pacman -Sy --noconfirm
if ! command -v curl &>/dev/null; then echo "Installing curl...";  pacman -S --noconfirm curl; fi
if ! command -v openssl &>/dev/null; then echo "Installing openssl..."; pacman -S --noconfirm openssl; fi
